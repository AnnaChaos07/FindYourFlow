...
1. Projektwurzel ist dieser Ordner (`/var/www/html`), nicht dessen Unterordner `timo`. In Git die neue Anwendung aufnehmen; das lokale Archiv `legacy-symfony` ist ausgeschlossen.
...
`npm run preview` stellt den tatsächlichen Export zusammen mit den Pages Functions auf Port 8788 bereit. `next start` ist für diesen statischen Export nicht vorgesehen. Docker ist optional: `docker compose up --build` startet die lokale Entwicklung mit Node, ohne PHP oder lokalen Datenbankdienst.
...
3. Mit dem vorbereiteten Admin-Zugang Inhalte bearbeiten. Impressum und Datenschutz sind als leere, unveröffentlichte Entwürfe vorbereitet. Anbieterangaben und passende Texte müssen ergänzt werden. Danach veröffentlichen und im Fußmenü verlinken.
